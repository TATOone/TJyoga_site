...
export const CAMPAIGN_ACCENTS: Record<CampaignAccentId, CampaignAccentDefinition> = {
  beginners: {
    ...
    heroHeadline: 'Практика, в которой вас не оставляют наедине с ковриком',
    heroSubhead:
      'Живые занятия в Zoom и записи, если не успели. Понятные уровни — не лента «делайте как получится». Преподаватель рядом в чате.',
    homeEmphasis:
      'Если вы давно хотели начать и откладывали из страха сделать «не так» — в клубе вас проводят, а не оценивают. Можно приходить, не зная санскрита и не имея «базы».',
    clubEmphasis:
      'Новичкам не нужно уже уметь. Утренние и вечерние практики рассчитаны на все уровни: преподаватель даёт опору и более простой вариант. Пропустили эфир — открываете запись. Если тело тревожит, пишете в чат.',
  ...
  depth: {
    ...
    heroSubhead:
      'Расписание на виду: утро, вечер, субботнее углубление. Рядом — библиотека записей, разборы техники и тексты. Классическая хатха, а не растяжка под плейлист.',
    homeEmphasis:
      'Если вам нужна система, а не бесконечная лента уроков — в клубе есть уровни, повторяемый ритм недели и работа с первоисточниками. Так практика держится дольше, чем вдохновение.',
    clubEmphasis:
  ...
  'office-restore': {
    ...
    heroSubhead:
      'Утро до созвонов и вечер после экрана. Если день съел — есть запись. Мягкий вход даже когда тело засижено, а голова не выключается.',
    homeEmphasis:
      'Если рабочий день оставляет сжатыми плечи, челюсть и поясницу — хатха в клубе рассчитана на живое тело, а не на «идеальную форму». Это не марафон гибкости.',
    clubEmphasis:
