...
export const processProdamusWebhook = async (params: {
  ...
  return params.store.runInTransaction(async (store) => {
    const eventResult = await store.recordPaymentEvent({
      dedupKey,
    ...
    if (params.payload.status === 'paid' && params.payload.provider_order_id) {
      const order = await store.getOrderByProviderOrderId(params.payload.provider_order_id);
      if (order) {
        const paidAt = params.payload.paid_at ?? new Date().toISOString();
        const paidOrder = await store.markOrderPaid(order.id);
        if (paidOrder) {
          ...
          const subscription = await store.upsertSubscriptionForOrder(paidOrder, paidAt);
          await store.saveSubscriptionEvent({
